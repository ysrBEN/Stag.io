...
import {
    LayoutDashboard, Search, FileText, User as UserIcon,
...
const StudentLayout = () => {
    ...
    const navLinks = [
        ...
        { name: 'Search Internships', path: '/student/search', icon: <Search className="w-5 h-5 mr-3" /> },
        { name: 'My Applications', path: '/student/applications', icon: <FileText className="w-5 h-5 mr-3" /> },
        { name: 'My Profile', path: '/student/profile', icon: <UserIcon className="w-5 h-5 mr-3" /> },
    ];
        ...
        <NavLink
            ...
            className={({ isActive }) =>
                `flex items-center px-4 py-3 my-1 rounded-r-lg transition-all border-l-4 ${isActive
                    ...
                    : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/10'
                }`
            }
        >
            {link.icon}
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-[#111827] flex font-sans">
      ...
      `}>
                ...
                <div className="p-4 border-t border-gray-200 dark:border-white/10 shrink-0">
                    <div className="flex items-center px-2 py-2 mb-4">
                        <div className="h-10 w-10 flex items-center justify-center rounded-full bg-teal-500 text-white font-bold shrink-0 overflow-hidden">
                            {user?.profilePicture ? (
                        ...
                        <div className="ml-3 overflow-hidden">
                            <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{user?.name || 'Student Name'}</p>
            ...
            {/* Desktop Header */}
            <div className="hidden md:flex fixed top-0 right-0 w-[calc(100%-16rem)] h-16 bg-white/80 dark:bg-[#111827]/80 backdrop-blur-md z-40 items-center justify-end px-8 border-b border-gray-200 dark:border-white/5 gap-3">
                <ThemeToggle />
                <NotificationBell />
            </div>
